onload = function(){
    ...
    var tNormal = vData_torus[1];
    var tColor = vData_torus[2];
    var tIndex = vData_torus[3];

    // vbo生成
    var vbo = new Array(3);
    vbo[0] = create_vbo(tPosition);
    vbo[1] = create_vbo(tNormal);
    vbo[2] = create_vbo(tColor);
    // vbo登録
	...
	uniLocation[2] = gl.getUniformLocation(prg, "lightPos");
	uniLocation[3] = gl.getUniformLocation(prg, "eyeDirection")
	uniLocation[4] = gl.getUniformLocation(prg, "lightAmbient");
    // 行列モジュールのオブジェクト生成
    var m = new matIV();
    ...
    var invMat = m.identity(m.create());

	// ビュー x プロジェクション座標変換行列 作成
	var eye = [0.0, 0.0, 15.0];
	var center = [0.0, 0.0, 0.0];
    m.lookAt(eye, center, [0.0, 1.0, 0.0], vMat);
    m.perspective(45, c.width / c.height, 0.1, 100, pMat);
    m.multiply(pMat, vMat, tmpMat);

    // 点光源の位置 設定
	var lightPosition = [-1.0, 1.0, 15.0];
	// 視点の方向 設定
	var eyeDirection = minusArr(eye, center);
	// 環境光 設定
	var ambientColor = [0.1, 0.1, 0.1, 1.0];// debug_so: これそのまま足すと彩度が+1.0されるからほんとはだめじゃない？
    ...
    (function(){
        ...
        // uniformLocationの登録
        gl.uniformMatrix4fv(uniLocation[0], false, mvpMat);
        gl.uniformMatrix4fv(uniLocation[1], false, invMat);
	...
	function torus(row, column, irad, orad){
		...
		for(var i = 0; i <= row; i++){
			...
			for(var ii = 0; ii <= column; ii++){
                ...
                var rz = rr * Math.sin(tr);// debug_so: z軸の方向は手前が正なので、マイナスを付けた。
                pos.push(tx, ty, tz);
                nor.push(rx, ry , rz);
	...
	// 配列同士の足し算
	function plusArr(data1, data2) {
		if (data1.count != data2.count) {return;}

		var newData = new Array(data1.count);
		for (var i in data1) {
	...
	// 配列要素の符号反転
	function inverseSignArr(data) {
		var newData = new Array(data.count);
		for (var i in data){
			newData[i] = - data[i];
		};
	...
	// 配列同士の足し算
	function minusArr(data1, data2) {
		if (data1.count != data2.count) {return;}
		return plusArr(data1, inverseSignArr(data2));
	};
};